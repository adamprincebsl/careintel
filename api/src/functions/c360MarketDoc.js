...
const paramsFrom = (q) => ({ market: q.get('market') ?? undefined, from: q.get('from') ?? undefined, to: q.get('to') ?? undefined });
...
app.http('marketDocRoster', {
  ...
  handler: async (request, context) => {
    ...
    if (!p.market) return { status: 400, headers: NO_STORE, jsonBody: { error: 'market is required' } };
    let data;
    try { data = await marketDocRoster(p); } catch (err) { return c360Fail(context, err); }
    try { await logAccess({ actor: principal, action: 'view-market-doc-roster', clientId: `market:${p.market}`, outcome: 'granted' }); }
